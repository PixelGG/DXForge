...
function Render.embeddedLogo(x, y, w, h, theme)
    ...
    if not logo or type(logo.Rows) ~= "table" then
        local drawX, drawY, drawW, drawH = Render.fitRect(x, y, w, h, 1)
        local unit = drawW / 100
        local metal = blend(theme.FontColor, {108, 112, 132}, 0.42)
        local darkMetal = blend(theme.SurfaceDarkColor or {9, 10, 15}, theme.FontColor, 0.18)
        local purple = theme.AccentColor
        local glow = theme.GlowColor

        local function thickLine(ax, ay, bx, by, color, thickness)
            thickness = thickness or math.max(1, math.floor(unit * 2))
            for offset = -thickness, thickness do
                Render.line({ax + offset, ay}, {bx + offset, by}, color)
                Render.line({ax, ay + offset}, {bx, by + offset}, color)
            end
        end

        Render.filled({drawX - 3, drawY - 3}, {drawX + drawW + 3, drawY + drawH + 3}, theme.ShadowColor)
        Render.filled({drawX, drawY}, {drawX + drawW, drawY + drawH}, theme.SurfaceDarkColor or {7, 8, 12})
        Render.filled({drawX + unit * 5, drawY + unit * 6}, {drawX + drawW - unit * 5, drawY + unit * 7}, theme.AccentDimColor or purple)

        local lx = drawX + unit * 12
        local ty = drawY + unit * 18
        local by = drawY + unit * 57
        local mid = drawY + unit * 38
        local thick = unit * 8

        Render.filled({lx, ty}, {lx + unit * 37, ty + thick}, metal)
        Render.filled({lx, ty}, {lx + thick, by}, metal)
        Render.filled({lx, by - thick}, {lx + unit * 37, by}, metal)
        Render.filled({lx + unit * 29, ty + unit * 7}, {lx + unit * 44, mid}, metal)
        Render.filled({lx + unit * 29, mid}, {lx + unit * 44, by - unit * 7}, metal)
        thickLine(lx + unit * 39, ty + unit * 8, lx + unit * 51, mid, purple, math.floor(unit * 2))
        thickLine(lx + unit * 51, mid, lx + unit * 39, by - unit * 8, glow, math.floor(unit * 2))

        local rx = drawX + unit * 55
        thickLine(rx, ty + unit * 2, rx + unit * 31, by - unit * 2, metal, math.floor(unit * 5))
        thickLine(rx + unit * 31, ty + unit * 2, rx, by - unit * 2, metal, math.floor(unit * 5))
        thickLine(rx + unit * 1, ty + unit * 7, rx + unit * 30, by - unit * 7, purple, math.floor(unit * 2))
        thickLine(rx + unit * 30, ty + unit * 7, rx + unit * 1, by - unit * 7, glow, math.floor(unit * 2))

        Render.filled({drawX + unit * 18, drawY + unit * 70}, {drawX + unit * 26, drawY + unit * 78}, purple)
        Render.text({drawX + unit * 29, drawY + unit * 68}, theme.FontColor, "DXForge")
        Render.filled({drawX + unit * 21, drawY + unit * 88}, {drawX + unit * 79, drawY + unit * 90}, theme.AccentColor)
        Render.filled({drawX + unit * 37, drawY + unit * 91}, {drawX + unit * 63, drawY + unit * 92}, theme.GlowColor)
        Render.box({drawX, drawY}, {drawX + drawW, drawY + drawH}, theme.AccentDimColor or theme.OutlineColor)
        return true
    end
...
function DXForge:renderStartup(theme)
    ...
    local bw, bh = 430 * scale, 306 * scale
    ...
    local logoBoxSize = 184 * scale
    local logoBoxX = x + (bw - logoBoxSize) / 2
    local logoBoxY = y + 34 * scale
    local logoBoxW = logoBoxSize
    local logoBoxH = logoBoxSize
